...
feature -- Test routines
	...
	integrator_basic_tests
		...
		do
			...
			assert_integers_equal ("has_111_you_may_have_more_or_less", 111, l_scanner.ecf_libraries.count)
			assert_integers_equal ("has_6_branches", 6, l_scanner.branch_count)
			assert_integers_equal ("has_9_leaves", 9, l_scanner.leaf_count)
			...
			assert_strings_equal ("the_branches", "logging_extension,pub_sub,randomizer,state_machine,test_extension,validation", the_branches (l_scanner.ecf_libraries))
			assert_strings_equal ("the_leaves", "ecfwingen,ecf_generator,framework,ehrapp,ig_test_project,integrator,permission,sav_training,test_set_bridge", the_leaves (l_scanner.ecf_libraries))
		end
